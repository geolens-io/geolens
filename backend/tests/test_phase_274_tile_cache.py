...
#
# settings.tile_database_url is a computed property (no setter), so we cannot
...
# exposes the dedicated tile_database_url attribute we want to test.


class _FakeSettings:
    """Minimal stand-in for the Settings instance used by pool._parse_dsn."""

    def __init__(self, tile_database_url: str | None) -> None:
        self.tile_database_url = tile_database_url
...
def test_parse_dsn_raises_when_tile_database_url_unset(monkeypatch):
    from app.processing.tiles import pool

    monkeypatch.setattr(pool, "settings", _FakeSettings(None))
    with pytest.raises(ValueError, match="tile_database_url"):
        pool._parse_dsn()

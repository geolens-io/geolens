...
class TestGraceWindowCannotRatchetOpen:
    """Queued rotations must preserve the first rotation's grace-window start."""
    ...
    async def test_concurrent_rotations_do_not_extend_the_retirement_cutoff(
        ...
        # rotated_at must stay near the first request's start. A queued caller
        # rewriting it would extend grace by at least the winner's 2.5s stall.
        # Allow scheduling noise while staying below that ratchet floor.
        overshoot = (row - started_at).total_seconds()
        tolerance = stall_seconds * 0.6  # 1.5s: >= 1s below the ratchet floor
        assert overshoot <= tolerance, (
            f"rotation timestamp moved {overshoot:.2f}s beyond the start "
            f"of the race, extending the {grace}s grace window "

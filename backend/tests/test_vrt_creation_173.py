...
def _stub_vrt_source_authz():
    """SEC-C (Phase 1172) / #1298: ``create_vrt_job`` now authorizes every
    source dataset against the caller via ``get_user_roles`` +
    ``check_datasets_access_bulk``. Those helpers issue their own
    ``db.execute`` calls, which would shift the call-count-ordered /
    single-return mock ``db`` sequences these pure unit tests rely on. Stub
    them to make ZERO ``db.execute`` calls (and always allow) so the existing
    sequences stay valid. The authorization behavior itself is covered by the
    DB-backed ``tests/test_vrt_source_authz_1172.py`` and
    ``tests/test_batch_dataset_authz_1298.py``. The stubs no-op when the
    function under test never reaches the authz block (e.g. the <2-sources
    guard), so this is safe to apply module-wide.
    """
    ...
    with (
        ...
        patch(
            "app.modules.catalog.authorization.check_datasets_access_bulk",
...
class TestVrtBuildFunctions:
    ...
    def test_build_vrt_needs_the_gdal_cli(self, tmp_path):
        """No in-process writer stands in for gdalbuildvrt, since it would open
        every source, and parse its CRS, in this process."""
        output = tmp_path / "out.vrt"
        with patch(
            "app.processing.raster.vrt.subprocess.run", side_effect=FileNotFoundError()
        ):
            with pytest.raises(FileNotFoundError):
                build_vrt("mosaic", [str(tmp_path / "a.tif")], str(output), "finest")

"""A pointcloud_dataset row is stored, searchable and counted, with no capability or vector export."""
...
@pytest.fixture
async def pointcloud_dataset(test_db_session):
    """A published public point cloud, inserted directly with the new values."""
    dataset, record_id, owner_id = await _owned_dataset(
        test_db_session, "pointcloud_dataset", "copc", None
    )
    yield dataset
...
async def test_a_stored_point_cloud_has_no_capability(pointcloud_dataset) -> None:
    """The stored row keeps both new values and gets the closed capability record."""
    assert pointcloud_dataset.source_format == "copc"
    caps = capabilities(pointcloud_dataset.record.record_type)
    assert pointcloud_dataset.record.record_type == "pointcloud_dataset"
...
async def test_quota_counts_a_point_cloud_as_a_dataset(
    test_db_session, pointcloud_dataset
) -> None:
    """Both quota aggregates count the point cloud toward its owner's datasets."""
    owner_id = pointcloud_dataset.record.created_by
...
async def test_search_filters_and_counts_point_clouds(
    client: AsyncClient, admin_auth_header: dict, pointcloud_dataset
) -> None:
    """The record_type filter accepts pointcloud_dataset and the facets count it."""
    resp = await client.get(
        "/search/datasets/",
        params={"record_type": "pointcloud_dataset"},
        headers=admin_auth_header,
    )
    assert resp.status_code == 200, resp.text
    assert str(pointcloud_dataset.id) in [f["id"] for f in resp.json()["features"]]

    facets = await client.get("/search/facets/", headers=admin_auth_header)
    assert facets.status_code == 200, facets.text
    assert facets.json()["record_type"]["pointcloud_dataset"] >= 1


async def test_a_point_cloud_record_advertises_no_vector_export(
    client: AsyncClient, admin_auth_header: dict, pointcloud_dataset, vector_dataset
) -> None:
    """A vector record lists the seven vector exports; a point cloud's lists none of them."""
    vector = await client.get(
        f"/collections/datasets/items/{vector_dataset.id}", headers=admin_auth_header
    )
    pointcloud = await client.get(
        f"/collections/datasets/items/{pointcloud_dataset.id}",
        headers=admin_auth_header,
    )
    assert vector.status_code == 200, vector.text
    assert pointcloud.status_code == 200, pointcloud.text

    vector_formats = vector.json()["properties"]["formats"]
    assert len(vector_formats) == 7
    assert "download_gpkg" in vector.json()["assets"]
    assert not set(vector_formats) & set(pointcloud.json()["properties"]["formats"])
    vector_assets = {"vector_tiles", "ogc_features"}
    assert not [
        key
        for key in pointcloud.json()["assets"]

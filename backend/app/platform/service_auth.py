...
# fix(#1770 round 35): the queue a header-auth service job goes on. A worker
# running the release before this one has no notion of a composed header
# line — its validator applies the bare-bearer charset directly to whatever
# `token` holds and refuses the first `:` or space, so a rolling deploy that
# enqueues from the upgraded API before every worker has upgraded fails a
# previously-working bearer WFS/OGC API import deterministically, after
# spending its single-use credential (round 34 finding, `ogr.py`'s
# `_sanitize_authorization_token` on the release before this PR takes no
# `service_format` and has no separator branch at all).
#
# A dedicated task NAME was the alternative and is not this: Procrastinate
# marks a job with no registered task FAILED on its own, before the task body
# ever runs, so the `ingest_jobs`/`refresh_runs` row this codebase owns is
# never touched and sits `pending` until the abandoned-run sweep — the exact
# hang #1689 and #1277 rejected the same alternative for at this door,
# already written down at `router_refresh.py`'s `_defer_refresh` and
# `service.py`'s `_defer_service`. A QUEUE a worker is not listening to is
# different in kind: nothing dequeues the job, nothing marks anything failed,
# and the row stays `pending` exactly as it would while every worker is busy
# — indistinguishable from ordinary backlog, and it resolves itself the
# moment any worker configured for both queues exists, which a rolling
# deploy produces within its own window rather than at a sweep cutoff.
#
# Retire this queue and the two things that reference it — this constant and
# the `worker_queues` default in `core/config.py` — once every deployed
...
def header_auth_job_queue(
    token: str | None, *, service_format: str | None
) -> str | None:
    """The queue a service-credentialed job belongs on, or None for the task's own.

    fix(#1770 round 35). ``token`` is what :func:`wire_credential` just
    composed: a finished header line for a header-auth format, the bare
    ArcGIS token otherwise, or None with no credential at all. Only the first
    is unreadable by a worker from the release before this one, so only that
    case routes to :data:`HEADER_AUTH_JOB_QUEUE`; ArcGIS and the
    no-credential case return None, which every call site reads as "the
    task's own queue, unchanged."

    Judged here, immediately after composition and before a caller may swap
    the line for a store reference (``credential_ref``), because the choice
    applies to both spellings alike: whichever one crosses the queue, the

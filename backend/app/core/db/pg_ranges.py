"""Value ranges for PostgreSQL's fixed-width numeric types (fix(#1778)).

A caller-supplied literal can be a perfectly good Python number and still be
wrong for the column it is compared against, and how that fails depends on
the SQL the compiler emits. Three shapes, all measured on PostgreSQL 18 with
asyncpg:

  - ``?ratio=1e100`` on a ``real`` column, through the property-filter path:
    SQLAlchemy's ``REAL()`` bind emits no narrowing cast, so Postgres
...
The first two silently answer a different question than the caller asked;
the third was an unhandled 500. Checking the range before the query makes
all three a 400 naming the property and the bound it broke, instead of
leaving the outcome to whichever cast the compiler chose.

``bigint`` is here for a fourth reason: a Python int outside int8 cannot be
encoded at all, and asyncpg's failure arrives as
``sqlalchemy.exc.DBAPIError`` — the base class, not ``DataError`` — so
nothing that only caught ``DataError`` ever saw it.
"""
...
def check_pg_value_range(pg_type: str, value: object) -> None:
    ...
    The message states the bound, so a 400 tells the caller which limit it
    broke rather than only that something was rejected. Types with no fixed
    width (``numeric``, ``double precision``, text, temporal) pass through.
    """
...
def check_int8_range(name: str, value: int) -> None:
    ...
    For pagination integers, which are caller-supplied and reach the driver
    untyped. FastAPI's ``int`` has no upper bound, so ``?offset=10**23`` used
    to surface as a 500 from the asyncpg encode path on both routers.
    """

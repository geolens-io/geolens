...
async def _validate_chat_layers(
    ...
    **Visibility:** does NOT filter by ``visible`` — that's a viewer-only
    decluttering signal, not "do not analyze this". A future visible-only
    scope needs an explicit ``include_hidden: bool``, not a silent change
    here.

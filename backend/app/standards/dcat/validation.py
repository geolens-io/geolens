"""W3C DCAT 3 structural validation.

The W3C DCAT 3 Recommendation does not publish an official JSON Schema for a
JSON-LD serialization, so this module applies structural / required-field
validation that mirrors the mandatory-property expectations of the
Recommendation. The report shape matches
``app.standards.dcat_us.validation.validate_dcat_us3`` and
``app.standards.geodcat_ap.validation.validate_geodcat_ap`` so all three
profiles share a consistent validation contract.
"""
...
def validate_dcat3(payload: Any, schema_name: str = "Catalog") -> dict:
    """Validate a payload against the W3C DCAT 3 structural expectations.
    ...
    if schema_name == "Catalog":
        ...
        errors = [
            _error(
                "$",
                "schema",
                f"Unknown DCAT 3 schema '{schema_name}'. Available: Catalog, Dataset",
            )
        ]
...
def _validate_catalog(payload: Any) -> list[dict]:
    ...
    if payload.get("@type") != CATALOG_TYPE:
        errors.append(
            _error("$.@type", "const", f"Catalog @type must be '{CATALOG_TYPE}'")
        )
...
def _validate_dataset(payload: Any, path: str) -> list[dict]:
    ...
    if payload.get("@type") != DATASET_TYPE:
        errors.append(
            _error(f"{path}.@type", "const", f"Dataset @type must be '{DATASET_TYPE}'")
        )

...
11. **Plugin configuration:**
    - If the `plugins` array exists, check each plugin ID against the registered plugin IDs from `frontend/src/components/map-plugins/register-plugins.ts` (currently `legend`, `measurement`). For a deep audit of the plugin platform itself, use `/plugin-audit`.
    - Flag unknown plugin IDs as `[LOW]`
    - If map has 3+ visible layers but no `legend` plugin → suggest adding it
...
7. **DOM inspection:**
   ...
   - Verify: layer legend is present (if the map has a legend plugin)
...
## Map Summary
<!-- ID, name, description, visibility, basemap, viewport, layer count, plugins, created/updated dates -->

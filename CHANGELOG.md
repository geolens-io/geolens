...
- Vector and cluster tile requests no longer queue without limit when the tile connection pool is
  saturated: a request that cannot get a connection within the pool's timeout answers 429 with
  `Retry-After` instead of waiting for as long as the client allows. (#1926)

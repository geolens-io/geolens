...
- [ ] 1050-02-defer-blob-revoke-PLAN.md — SF-05 / SMOKE-09: defer thumbnail blob URL revoke until image loads / unmount
- [ ] 1050-03-gate-anonymous-probes-PLAN.md — SF-06 / SMOKE-10: gate anonymous pre-auth probes (enabled: !!token + isAdmin)
...
**Headline wins:** Inline fix for v1010 PERF-03 bulk-delete UI regression (`c4576717` — outside-click guard cleared selection before confirm-click could dispatch onBulkDelete); inline fix for render-mode swap leaking line layout keys into the circle adapter (`8713b73f` — Line→Arrow MapLibre `unknown property` errors); inline fix for StyleJsonDialog defective `lazy()` (`3df84554` — chunk now resolves on first open, not on mount). Backend bulk-delete endpoint regression-checked via direct fetch. Full details: [milestones/v1010.1-ROADMAP.md](milestones/v1010.1-ROADMAP.md).

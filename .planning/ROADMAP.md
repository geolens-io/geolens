...
## v1033 Builder Terrain, Label & Render-Mode QA

### Milestone Goal

Close the builder render-mode persistence defects surfaced by a live Playwright MCP walkthrough of the two ADK sample maps — chiefly the DEM `render_mode:'terrain'` strip-on-load — and add the missing layer-list label indicator, with light builder polish and a raster-cache hygiene fix.
...
**Goal**: DEM and raster layers restore their saved render mode on every map load — `'terrain'` and `'image'` survive the normalize round trip so the 3D terrain mesh attaches and "Render as" never silently reverts to Image.
**Depends on**: Nothing (first phase)
**Requirements**: RMODE-01, RMODE-02, RMODE-03
**Success Criteria** (what must be TRUE):
  1. Map A (`8dd6a129`) fresh load: `map.getTerrain()` is non-null and a `terrain-dem` source exists — the 3D relief mesh is visible without any user interaction.
...
**Goal**: Users can see at a glance which layers in the builder layer list have labels enabled, without opening the label editor.
**Depends on**: Phase 1148
...
**Goal**: The point-layer editor shows one "Render as" control instead of two; enabling DEM hillshade on a terrain-bound DEM no longer floods the console with `backfillBorder` errors; and the raster tile worker's band-stats cache is bounded.
**Depends on**: Phase 1148
...
**Plans**: 3 plans
- [x] 1150-01-PLAN.md — Remove redundant point render-as dropdown (POLISH-01)
- [x] 1150-02-PLAN.md — DEM hillshade dual-consumer guard + advisory note (POLISH-02)
- [x] 1150-03-PLAN.md — Bound _band_stats_cache with LRUCache(maxsize=256) (HYG-01)
...
**Depends on**: Phase 1148, Phase 1149, Phase 1150
**Requirements**: QA-01, QA-02
**Success Criteria** (what must be TRUE):
  1. Orchestrator-driven Playwright MCP on Map A (`8dd6a129`): fresh load shows 3D terrain mesh (non-null `getTerrain()` + visible relief); saving a render-mode change then reloading preserves the selection; labeled layer rows show the indicator and unlabeled rows do not; reorder and visibility toggle work cleanly; 0 console errors. (NOTE: QA-01 requires live `mcp__playwright__*` tools — only the orchestrator can drive this; subagents must NOT attempt to fabricate MCP evidence.)
  2. Orchestrator-driven Playwright MCP on Map B (`c39be324`): DEM hillshade renders cleanly with 0 console errors; layer controls work; 0 console errors.
  3. All standard code gates pass: frontend `npm run typecheck` exits 0, `vitest` (touched suites) passes, `e2e:smoke:builder` passes, i18n check 2/2, backend `pytest` (raster and tile suites), `make openapi-check` reports no drift (render_mode change is frontend-type-only; no backend schema change).
  4. CHANGELOG has a v1033 entry describing the render-mode fix, label indicator, builder polish, and cache hygiene.
**Plans**: TBD

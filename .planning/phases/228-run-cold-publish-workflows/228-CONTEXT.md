...
- PyPI Trusted Publishing is configured for `geolens-sdk` AND `geolens` projects via the PyPI web UI ("Add a new pending publisher"). Required fields: PyPI Project Name, GitHub repository owner, repository name, workflow filename (`publish-sdks.yml` for the SDK; `publish-cli.yml` for the CLI), and environment name (none — repo-level OIDC). The workflows already declare `permissions: id-token: write` (`publish-sdks.yml:25`, `publish-cli.yml:18`), so no YAML change is required for the OIDC handshake — just remove the `UV_PUBLISH_TOKEN` env injection and let `uv publish` use `--trusted-publishing automatic` (uv 0.5+) or fall back to `pypi-publish` action with `attestations: true`.
- `publish-sdks.yml` runs end-to-end on `main` once via `workflow_dispatch` with `target=both, dry_run=false` after a preceding `dry_run=true` validation. `geolens-sdk@1.0.0` is installable from PyPI; `@geolens/sdk@1.0.0` is installable from npm.
- `publish-cli.yml` runs end-to-end on `main` once via `workflow_dispatch` with `dry_run=false` after a preceding `dry_run=true` validation. `geolens@1.0.0` is installable from PyPI; `pip install geolens && geolens --version` returns `1.0.0` on a fresh Python 3.11+ environment.
...
- A pre-flight package-name availability check is run BEFORE the first hot publish: `pip index versions geolens-sdk` and `pip index versions geolens` and `npm view @geolens/sdk version` and `npm view geolens version`. Each must return either "no versions" / `404` (name unclaimed — first publish will claim it) or a version `< 1.0.0` (we'd be the first 1.0.0). If any returns `1.0.0` or higher, STOP — the name is already claimed by someone else and either (a) we don't own it (need to contest via PyPI/npm support) or (b) we DO own it but a prior publish already happened (re-check provenance before proceeding).

**In scope:** configure PyPI Trusted Publishing for `geolens-sdk` and `geolens` projects (web-UI step, documented in VERIFICATION.md); add `NPM_TOKEN` repo secret (manual, documented in VERIFICATION.md); update `publish-sdks.yml` Python job to drop `UV_PUBLISH_TOKEN` env in favor of `uv publish --trusted-publishing automatic` (or migrate to `pypa/gh-action-pypi-publish@release/v1` with attestations); update `publish-cli.yml` similarly; pre-flight name-availability check (one-time bash script committed to `.github/scripts/preflight-publish.sh` or inline in workflow); run `dry_run=true` once per workflow to validate build artifacts; run `dry_run=false` once per workflow to publish; create `.github/workflows/verify-published.yml` for the clean-machine install verification (Python 3.13 docker + Node 22 docker, three `pip install`/`npm install` smoke checks); update `docs/sdks.md` and `docs/cli.md` to reflect the new auth pattern (Trusted Publishing for PyPI, token for npm); write `.planning/phases/228-run-cold-publish-workflows/228-VERIFICATION.md` capturing all four success criteria with workflow run URLs, install command output, and the secret/publisher inventory.
...
- **D-01 — PyPI Trusted Publishing (OIDC) for both `geolens-sdk` and `geolens` projects.** No long-lived `PYPI_TOKEN` secret. Reason: the workflows already declare `permissions: id-token: write` (look at `publish-sdks.yml:25` and `publish-cli.yml:18` — the comments explicitly call out "for PyPI Trusted Publishing (future migration)"); migrating now is the lowest-friction time because the secret doesn't exist yet (skip the create-then-rotate-then-delete cycle). Trusted Publishing eliminates token-leak risk, removes the rotation burden, and is PyPI's officially recommended pattern as of 2024+. The PyPI web-UI configuration is per-project: `geolens-sdk` and `geolens` each need a "pending publisher" entry pointing at this repo + the corresponding workflow filename. Acceptance: `gh secret list --repo geolens-io/geolens | grep PYPI_TOKEN` returns nothing AND a manual workflow-dispatch run of `publish-sdks.yml` (Python target, dry_run=false) succeeds AND PyPI shows the published `1.0.0` version with provenance attestations.

- **D-02 — Classic `NPM_TOKEN` repo secret for npm.** npm's OIDC trusted-publishing is still beta as of 2026-05 and not all features are stable. Generate an "Automation" token (recommended over "Publish" tokens for CI per npm docs at <https://docs.npmjs.com/creating-and-viewing-access-tokens>) scoped to the `@geolens` org. Add via `gh secret set NPM_TOKEN --body $TOKEN --repo geolens-io/geolens` (or via the GitHub web UI). Token rotation is a manual operational concern; document in `docs/sdks.md` that `NPM_TOKEN` should be rotated annually or when any maintainer leaves the org. Acceptance: `gh secret list --repo geolens-io/geolens | grep NPM_TOKEN` returns one row dated 2026-05 or later AND a manual workflow-dispatch run of `publish-sdks.yml` (TypeScript target, dry_run=false) succeeds AND npm shows the published `@geolens/sdk@1.0.0` as `latest`.
...
- **D-05 — Pre-flight name-availability check.** Before the first hot publish, run a small bash script committed to `.github/scripts/preflight-publish.sh` (or inline in the workflow as a pre-step) that runs:
  ```bash
  pip index versions geolens-sdk 2>&1 | grep -q "No matching" && echo "geolens-sdk: unclaimed" || echo "geolens-sdk: TAKEN — investigate"
  ...
  ```
  Fail the job (`exit 1`) if ANY of `geolens-sdk` (PyPI), `geolens` (PyPI), or `@geolens/sdk` (npm) shows a version `>= 1.0.0` we don't recognize. This is a one-time check; once `1.0.0` is published, future publishes will obviously find a version present (the check should be skipped or downgraded to "informational" after first publish). Plan picks: keep it as a hard gate forever (catches unauthorized publishes from outside the repo) OR convert to informational after first publish. Recommended: keep as hard gate with a `force=true` workflow input to override.
...
- **D-07 — Separate workflow `.github/workflows/verify-published.yml` for clean-machine smoke checks.** Two jobs: `verify-python` (runs `docker run --rm python:3.13-slim sh -c "pip install geolens-sdk geolens && geolens --version && python -c 'from geolens_sdk import GeolensClient; print(GeolensClient)'"`) and `verify-typescript` (runs `docker run --rm node:22-slim sh -c "npm install @geolens/sdk && node -e 'import(\"@geolens/sdk\").then(m => console.log(typeof m.GeolensClient))' "`). Trigger: `workflow_dispatch` (manual) AND on push of a `release/*` tag (future-proofing — even though phase 228 doesn't introduce release tags yet, adding the trigger now means future tag pushes auto-verify). Reason for separate workflow: (1) re-runnable independently of publishing — useful for "did the package break for a downstream user?" checks; (2) shorter, tighter blast radius — failures don't tangle with the publish workflow's success/failure semantics; (3) idiomatic GitHub Actions pattern (one workflow per concern). Acceptance: `verify-published.yml` exists, has been run at least once with `dry_run=false`, and both jobs exit 0.

- **D-08 — Clean-machine env minimums.** Python verification uses `python:3.13-slim` (matches the publish workflow's `actions/setup-python@v5: "3.13"` minor) — 3.13 is the highest in our `classifiers` for both `geolens-sdk` and `geolens`, so verifying against the highest is the strongest signal. We do NOT verify against 3.10/3.11/3.12 in this phase (deferred — that's a matrix-build job and adds CI minutes); the `requires-python = ">=3.10"` (geolens-sdk) and `>=3.11"` (geolens) constraints are trusted to be honored by PyPI's resolver. npm verification uses `node:22-slim` (matches `setup-node@v6: 22` in `publish-sdks.yml:69` and the package's `"engines": { "node": ">=18" }` declaration). Plan can later add a verification matrix if desired.
...
### Existing workflow files (target of refactor)
- `.github/workflows/publish-sdks.yml` (~95 lines) — already declares `permissions: id-token: write`; already supports `workflow_dispatch` with `target` and `dry_run` inputs; current Python publish step uses `UV_PUBLISH_TOKEN` env (`:60-61`); current TypeScript publish step uses `NODE_AUTH_TOKEN` env (`:88-89`); first-publish-ready (`--access public` flag set, `publishConfig.access=public` in package.json — RESEARCH Pitfall 10 from Phase 215).
- `.github/workflows/publish-cli.yml` (~48 lines) — already declares `permissions: id-token: write` (`:18`); already supports `workflow_dispatch` with `dry_run` input; current PyPI publish step uses `UV_PUBLISH_TOKEN` env (`:48-49`).
...
- `sdks/python/pyproject.toml` — `name = "geolens-sdk"`, `version = "1.0.0"`, `requires-python = ">=3.10"`, `license = "Apache-2.0"`, `build-backend = "hatchling.build"`. Versions stay (D-06).
...
- `cli/pyproject.toml` — `name = "geolens"`, `version = "1.0.0"`, `requires-python = ">=3.11"`, `license = "Apache-2.0"`. Versions stay.
...
- `README.md` — verify (don't necessarily change) that the install instructions (`pip install geolens-sdk`, `npm install @geolens/sdk`, `pip install geolens`) are present and accurate. README is the source for PUBLISH-04 verification.
...
- **uv-first toolchain** — every Python step in CI uses `uv` (build, sync, publish). Stay consistent in `verify-published.yml` Python job: `pip install geolens-sdk` is fine for the verification (mimics what a downstream user does), NOT `uv add` (that's project-management ergonomics, not a public-install smoke test).

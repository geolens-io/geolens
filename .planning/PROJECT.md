...
## Current Milestone: v1011 Map Builder Polish & Bug Sweep

**Goal:** Close 11 user-reported Map Builder polish/bug items (5 broken affordances, 3 small-screen layout collisions, 3 UX decisions) via Playwright MCP inspect-verify-fix loop on live `localhost:8080` stack; surface and triage emergent issues found in flight.

**Target features:**
- Fix broken affordances: regular layer visibility toggle (repro at `/maps/c868cc3a-a3a0-4714-b559-67b3f2b478e2`), delete layer, "rename group" focus-on-input, plus DETAIL LEVEL disposition (investigate → propose remove vs fix)
- UX clarifications: enlarge layer-group expand caret, replace per-sublayer opacity slider with config-state indicators (label, filters, etc.), make basemap row draggable in layer order, convert Map Settings Widgets section to enable/disable availability toggles
- Small-screen resilience: right sidebar must not collapse over zoom controls; lat/long/zoom pill must not overlap map-widget container; basemap selector + audited right-sidebar elements must not stack duplicate "X" close buttons
- Emergent-findings clause: any other issues surfaced during Playwright MCP inspection get triaged inline (fix-now vs defer-with-rationale)

**Shape:** Hygiene-milestone pattern (single phase 1051, sequential plans, single CTRL-01 close gate) per `feedback_hygiene_milestone_pattern.md`. Mirrors v1009.1 (Phase 1045) and v1010.2 (Phase 1050). Execution path: `/gsd-autonomous` with Playwright MCP for live verification.

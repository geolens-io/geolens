...
**Goal delivered:** Closed the builder render-mode persistence defects surfaced by a live Playwright MCP walkthrough of the two ADK sample maps — chiefly the DEM `render_mode:'terrain'` strip-on-load (no 3D mesh + raster "Render as" save-revert) — added the missing layer-list label indicator, and shipped light builder polish + a raster-cache hygiene fix.

**Delivered:** 9/9 requirements across phases 1148-1151 (7 plans). **Render-mode persistence (RMODE-01/02/03):** root cause was the `RENDER_MODES` allowlist (`normalize-style-config.ts:92`) omitting `'terrain'`/`'image'`, so `normalizeRenderMode()` stripped `'terrain'` on every load → terrain mesh never attached + raster "Render as" reverted to Image. Fix added both to the allowlist + the `StyleConfig['render_mode']` union (`api.ts`, hand-maintained — no OpenAPI regen) and removed the `DEMEditorScene` BSR-09 boundary cast/comment, with round-trip + RENDER_MODES-guard unit tests. **Label indicator (LABEL-01):** derived `Type`-glyph indicator on `StackRow` rows where `label_config.column` is set (mirrors the `map-sync` label gate), 4-locale i18n + a11y title. **Polish/hygiene (POLISH-01/02, HYG-01):** removed the redundant point render-as dropdown (segmented control is now sole picker); `isHillshadeTerrainBound` guard skips the hillshade raster-dem consumer when a DEM powers active terrain (stops `backfillBorder` spam; inactive when terrain off, so primary hillshade path is safe) + DEM-editor advisory note; `_band_stats_cache` → `cachetools.LRUCache(maxsize=256)`. **Close-gate (QA-01/02):** orchestrator-driven live Playwright MCP on both maps (Map A terrain on fresh load + DEM editor shows Terrain + label indicator + single render-as control; Map B hillshade renders with terrain off; 0 console errors each) + typecheck 0 / vitest 2601 / i18n 2 / lint 0-err / backend raster·tile 76 / `make openapi-check` no-drift / `e2e:smoke:builder` 26. Milestone audit `tech_debt` (integration CLEAN 9/9 links + 4/4 E2E flows; 0 blockers). **Deferred (Future):** multi-band stretch (RASTER-STRETCH-03), configurable percentile/σ (RASTER-STRETCH-UI-01), `band_count` cosmetic on get_dataset_meta (RASTER-META-01), non-DEM single-band raster fixture (TESTDATA-01). Still deferred: CI-01 GH Actions billing (ops task).
...
**Goal delivered:** Closed the v1030 builder carry-forward backlog — four new render-mode editor controls, shared-link social cards, and SharePanel typography hygiene — proven on the live builder via orchestrator-driven Playwright MCP smoke.

**Delivered:** 8/9 requirements across phases 1140-1143 (8 plans). Hypsometric tint (`color-relief-sync.ts` preset ramp picker, EDITOR-DEM-05); single-band raster colormap (RasterEditor COLORMAP gated on `band_count===1` → `buildColormapTileUrl` → Titiler proxy Literal/frozenset allowlist + nginx cache-key, EDITOR-RASTER-COLORMAP); built-in fill-pattern editor (`FillPatternPicker` + idempotent sprite registrar wired into the fill adapter, EDITOR-FILL-01); OG-image social cards via Path A (migration 0024 `og_image_uri`, owner-only `PUT`/public `GET /maps/{id}/og-image/`, public `GET /maps/shared/{token}/card` HTML meta route with HTML-escaping + absolute URLs + 404 on non-public, 1200×630 capture in the existing single `doCapture` repaint, no `@vercel/og`/`satori`, SHARE-08); SharePanel reduced to ≤2 font weights (SHARE-10); and a QA close-gate (live Playwright MCP; typecheck 0 / vitest 2599/2599 / pytest 181/181 / e2e:smoke:builder 26/26 / i18n 2/2; OpenAPI + Python/TS SDK refresh; CHANGELOG [1.6.0]). Milestone audit graded `tech_debt`; cross-phase integration verified CLEAN (12/12 links, 4/4 E2E flows). **Deferred to v1032:** EDITOR-DEM-04 contour overlay — `maplibre-contour` worker emits ~28 MapLibre error events on enable; UI gated off (`CONTOUR_CONTROL_ENABLED=false`), `contour-sync.ts` + 5 unit tests dormant, one-boolean re-enable. Still deferred: CI-01 GH Actions billing (operator ops task).

## Recent Shipped Milestone: v1030 Map Builder Polish Sweep

**Shipped:** 2026-05-28
**Tag:** local `v1030`

**Goal delivered:** Systematically walked every existing map across layer types and proved builder/viewer/AI/sharing quality via live Playwright MCP, closing surfaced gaps and shipping easy-win UX enhancements.

**Delivered:** 44/44 requirements across phases 1133-1139 (38 plans). Key outputs: audit-first `BUILDER-WALKTHROUGH-AUDIT.md` (P0/P1/P2 triage); Tier-1 map bug fixes (delete-layer, visibility-toggle, rename-group focus) + ≤800px layout collision fixes; AI confirm-before-apply Shape-B staging buffer (`chat-action-staging.ts` above `dispatchLayerAction`, `BuilderActionSource` union unchanged) with action preview chips + viewport-aware suggestions + inline data-analysis card; per-render-mode editor polish (RasterEditor 7 sliders, LineEditor cap/join LAYOUT, FillEditor extrusion hint, BasemapEditor "No basemap" preset); share chips / expiration presets / "Powered by GeoLens" branding / legend+title export / conditional iframe preview (advanced sharing enterprise-gated); easy-wins (Cmd/Ctrl+S, popup URL/media, empty-layer hint); and a 3-viewport Playwright MCP close-gate (1440×900 / 800×600 / 414×896) + disabled-AI smoke. Milestone audit PASSED (10/10 integration, 0 blockers). Carry-forward consumed by v1031: SHARE-08 OG-cards, F2 font hygiene. Still deferred: CI-01 GH Actions billing (ops task).

## Recent Shipped Milestone: v1029 DCAT 3.0

**Shipped:** 2026-05-27

**Goal delivered:** Added full DCAT-US Schema v3.0 support so GeoLens can emit, validate, document, and operationalize federal `data.json` metadata while preserving existing catalog visibility rules.
...
**Delivered:** 25/25 requirements across phases 1129-1132. Key outputs: official schema foundation, DCAT-US profile serializer, explicit profile export routes, schema-backed validation report routes, docs/migration notes, OpenAPI/SDK refresh, focused backend gates, and Playwright MCP API runtime verification.

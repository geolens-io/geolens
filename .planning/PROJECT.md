...
## Current Milestone: v1031 Builder Render-Mode & Share Polish

**Goal:** Close the v1030 builder carry-forward backlog — add four new render-mode editor controls, complete shared-link social cards, and tidy SharePanel typography — then prove it all on the live builder via intensive Playwright MCP smoke.

**Target features:**
- Render-mode controls (999.18 feature register, render-mode sub-group only):
  - **EDITOR-FILL-01** — fill-pattern styling (sprite upload + pattern selection flow) for fill render mode.
  - **EDITOR-DEM-04** — contour-line overlay control for DEM/terrain layers.
  - **EDITOR-DEM-05** — hypsometric tint color ramps for terrain.
  - **EDITOR-RASTER-COLORMAP** — single-band stretch + colormap UI for raster layers (may require backend colormap render-path scoping at plan time).
- Social cards (999.17 / SHARE-08) — OG-image / social-card meta on shared map links. Pick Path A (separate 1200×630 capture + nullable `og_image_uri` column + upload/serve routes) vs Path B (backend resize from the native canvas capture) in a planning audit. Do NOT add `@vercel/og` or `satori` (STACK do-not-add list).
- SharePanel font-weight hygiene (999.19 / F2) — reduce SharePanel from 3 font weights across 5 sites to ≤2 per the UI-SPEC max-2 rule. Cosmetic.
- Close-gate — intensive live Playwright MCP smoke of the builder against `http://localhost:8080/maps/8dd6a129-8eb0-4ba9-b421-716c83b160dd`, plus focused frontend/backend tests on touched surfaces, `e2e:smoke:builder` + i18n parity, CHANGELOG, and OpenAPI/SDK refresh where backend changed.

**Key context:**
- Phase numbering continues from **1140** (no `--reset-phase-numbers`). Branch `codex/builder-polish-walkthrough` (177 commits ahead of main; v1030 lives on this branch, not yet merged to main).
- Execution method: `/gsd-autonomous` for run-to-end (discuss → plan → execute per phase). Playwright MCP is the canonical close-gate per v1027/v1028/v1029/v1030 precedent.
- **Playwright MCP is orchestrator-only** — GSD subagents (gsd-executor etc.) lack `mcp__playwright__*` tool access. The orchestrator MUST drive all live MCP verification directly; subagents asked to "run MCP" fabricate PASS/FAIL or write `.spec.ts` files instead.
- Substrate: v1026 style reconciler + v1027 controller/action/sync boundaries + per-render-mode editors (v1010 split, v1030 polish). New controls extend the owned-property contracts; behavior preservation is the default for existing surfaces.
- External research intentionally skipped (internal feature-add; per-phase technical research happens at plan-phase via gsd-phase-researcher — e.g., Titiler single-band colormap path for EDITOR-RASTER-COLORMAP).
- Pre-commit hooks remain ON for all commits.
- Out of scope for v1031: 999.18 editor-convenience sub-group (categorical icon mapping, custom basemap style URL override) and layer-type expansion (text/annotation, draw, LiDAR) — stay parked in the 999.18 backlog register; builder architecture rewrites; new LLM provider integrations; marketing/docs site work; new connector backends; enterprise edition changes; the open-core/Cloud backlog (999.6/13-16).
...
---
*Last updated: 2026-05-28 — started milestone v1031 Builder Render-Mode & Share Polish. Scoped the v1030 builder carry-forward backlog: four render-mode editor controls (fill-pattern, DEM contour, hypsometric tint, single-band raster colormap), SHARE-08 OG-image/social-card meta, F2 SharePanel font-weight hygiene, and an intensive Playwright MCP builder close-gate against map `8dd6a129`. Phase numbering continues from 1140; running via `/gsd-autonomous`. Branch `codex/builder-polish-walkthrough` (177 commits ahead of main).*

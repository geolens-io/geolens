...
> **Resolution (2026-05-31, milestone-close):** The single open gap below (TOOL-02) was fixed by hand at milestone-close in commit `ce9c3e0`. The verdict is upgraded from `tech_debt` → **passed** at a genuine **19/19**. The original `tech_debt` finding and its evidence are preserved verbatim below for the record; the TOOL-02 row and carry-forward list now reflect the resolved state.
...
| QA close-gate | QA-01 | **PARTIAL / see note** | Live-MCP evidence artifact (`v1036-1165-admin-map-plugins.png` present in repo root) and commit `1a54aff8 docs(1165): live MCP close-gate PASSED (QA-01)`. Full file-level confirmation of `1165-.../evidence/LIVE-MCP-EVIDENCE.md` could not be completed due to a tooling stall (see Limitations). DB round-trip substrate independently confirmed PASS above. |
...
1. **TOOL-02 (skills rename) — RESOLVED 2026-05-31 (commit `ce9c3e0`).** Renamed `.agents/skills/geolens-widget-audit/` → `geolens-plugin-audit/`, rewrote `SKILL.md` to plugin vocabulary, and repointed its references to the live `.claude/commands/plugin-audit.md` and `frontend/src/components/map-plugins/` + `register-plugins.ts`. `grep -rni widget .agents/skills/geolens-plugin-audit/` → 0. No longer a carry-forward.

...
- 🚧 **v1010.1 Live Playwright MCP Smoke** — Phase 1049 (in progress)

## Phases

### 🚧 v1010.1 Live Playwright MCP Smoke (Phase 1049)

**Milestone Goal:** Drive a fresh-stack, interactive Playwright MCP smoke check against v1010's headline win surfaces (lazy-load, debounce/rAF coalesce, bulk-delete batching, LayerStyleEditor split, popup_config error toast); classify findings P0/P1/P2; ship P0/P1 inline or defer-with-rationale.

- [ ] **Phase 1049: mcp-smoke-verification** — Rebuild stack fresh, drive interactive MCP smoke across 5 v1010 win surfaces, file SMOKE-FINDINGS.md, fix P0/P1 inline or defer with rationale

### Phase 1049: mcp-smoke-verification
**Goal**: Fresh-stack interactive Playwright MCP smoke check against v1010 win surfaces produces a SMOKE-FINDINGS.md with classified findings (P0/P1/P2) for every observed gap; P0/P1 either ship inline or carry deferral rationale; final post-fix smoke confirms no regression.
**Depends on**: Nothing (v1010 already shipped + tagged)
**Requirements**: SMOKE-01, SMOKE-02, SMOKE-03, SMOKE-04, SMOKE-05, SMOKE-06, SMOKE-07
**Success Criteria** (what must be TRUE):
  1. `docker compose down -v` then `docker compose up -d --build` ran cleanly; backend `/api/health` 200; frontend `http://localhost:8080` responsive.
  2. Playwright MCP browser session authenticated as admin and reached Map Builder on a representative saved map.
  3. All 5 v1010 win surfaces exercised with MCP and recorded (screenshot + console + network).
  4. SMOKE-FINDINGS.md exists with P0/P1/P2-classified findings and disposition per finding.
  5. P0 + P1 findings shipped or escalated; no silent skips.
  6. Console-error budget respected (zero unhandled errors).
  7. Post-fix re-smoke confirms no regression (skipped if no inline fixes shipped).
**Plans**: 1 plan

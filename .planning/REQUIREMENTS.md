...
- [ ] **SMOKE-09**: Post-login redirect to `/` produces zero `net::ERR_FILE_NOT_FOUND` console errors for `blob:` thumbnail URLs. Closes SF-05. Fix path: defer `URL.revokeObjectURL(blob)` until the `<img>` finishes loading OR move revoke to component unmount cleanup. Locate via `git grep "revokeObjectURL" frontend/src`.

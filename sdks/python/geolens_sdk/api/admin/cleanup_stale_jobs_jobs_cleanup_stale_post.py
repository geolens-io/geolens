...
from ...models.stale_cleanup_response import StaleCleanupResponse
...
) -> ProblemDetail | StaleCleanupResponse | None:
    if response.status_code == 200:
        response_200 = StaleCleanupResponse.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ProblemDetail | StaleCleanupResponse]:
    return Response(
...
def sync(
    ...
    **Ops-only.** Not used by the GeoLens UI — invoke from `curl`/`gh api`/cron
    ...
    Returns:
        ProblemDetail | StaleCleanupResponse
    """

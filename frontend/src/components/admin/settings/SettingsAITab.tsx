          ...
          {canManageUsers && embeddingStats && (
            <div className="rounded-lg border p-4 max-w-md space-y-3">
              ...
              <div className="flex gap-2">
                {/* fix(#1503): stale records already have a row, so the
                    non-force backfill skips them. Offer this button only for
                    records that have no embedding at all. */}
                {embeddingStats.missing_records - embeddingStats.stale_records > 0 && (
                  <Button
                    variant="outline"
                    size="sm"
                    className="flex-1"
                ...
                <div className="flex items-start gap-2 rounded-md border border-warning/30 bg-warning/5 p-3">
                  ...
                  <p className="text-sm text-foreground">
                    {t('ai.staleEmbeddingsWarning', {
                      ...
                      defaultValue: '{{count}} embedding(s) were generated by a different model and cannot be used by semantic search. Regenerate all embeddings to restore coverage.',
                    })}
                  </p>
                </div>
              )}

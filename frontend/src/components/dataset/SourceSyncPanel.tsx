      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          <dl className="grid gap-3 sm:grid-cols-2">
            ...
            <div><dt className="text-xs text-muted-foreground">{t('sourcePanel.sync.lastRun')}</dt><dd className="mt-1 text-sm font-medium">{automation.last_occurrence ? <>{automation.last_occurrence.scheduled_for ? formatDateTimeUtc(automation.last_occurrence.scheduled_for) : t('sourcePanel.sync.noRuns')}<span className="text-muted-foreground"> · {t(`sourcePanel.sync.occurrenceState.${automation.last_occurrence.state}`)}</span></> : t('sourcePanel.sync.noRuns')}</dd></div>
          </dl>
